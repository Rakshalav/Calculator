...
std::vector<wxButton*> MainFrame::CreateButtons()
{
	...
	{
		wxButton* btn = new wxButton(panel, ArrayButtonID[i], ArrayButtonLabels[i], wxDefaultPosition, wxSize(Btn_Width_Height, Btn_Width_Height));
		btn->SetBackgroundColour(ArrayButtonColors[i]);
		btn->SetForegroundColour(LightTheme.TextColor);
		btn->Bind(wxEVT_BUTTON, &MainFrame::OnButtonPressed, this, ArrayButtonID[i]);

		// Assign to struct and array
...
void MainFrame::SetUpSizers(std::vector<wxButton*> Buttons, wxFont font)
{
...
void MainFrame::OnButtonPressed(wxCommandEvent& evt)
{
	...
	switch (id)
	{
	case MainFrame::ID_BTN_ZERO:         AppendSymbol(Label.Zero); break;
	case MainFrame::ID_BTN_ONE:          AppendSymbol(Label.One); break;
	case MainFrame::ID_BTN_TWO:          AppendSymbol(Label.Two); break;
	case MainFrame::ID_BTN_THREE:        AppendSymbol(Label.Three); break;
	case MainFrame::ID_BTN_FOUR:         AppendSymbol(Label.Four); break;
	case MainFrame::ID_BTN_FIVE:         AppendSymbol(Label.Five); break;
	case MainFrame::ID_BTN_SIX:          AppendSymbol(Label.Six); break;
	case MainFrame::ID_BTN_SEVEN:        AppendSymbol(Label.Seven); break;
	case MainFrame::ID_BTN_EIGHT:        AppendSymbol(Label.Eight); break;
	case MainFrame::ID_BTN_NINE:         AppendSymbol(Label.Nine); break;

	case MainFrame::ID_BTN_EXPONENT:     AppendSymbol(Label.Exponent); break;
	case MainFrame::ID_BTN_POWER2:       AppendSymbol(Text.Power2); break;
	case MainFrame::ID_BTN_ROOT:         SQRT_button(); break;

	case MainFrame::ID_BTN_DIVIDE:       AppendSymbol(Label.Divide); break;
	case MainFrame::ID_BTN_MULTIPLY:     AppendSymbol(Label.Multiply); break;
	case MainFrame::ID_BTN_ADD:          AppendSymbol(Label.Add); break;
	case MainFrame::ID_BTN_SUBTRACT:     AppendSymbol(Label.Substract); break;
	...
	case MainFrame::ID_BTN_CLOSEBRACKET: AppendSymbol(Label.BracketClose); break;
...
void MainFrame::OnBtnDel_pressed() {
	long from, to;
	input->GetSelection(&from, &to);
